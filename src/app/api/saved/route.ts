...
export async function GET(request: Request) {
  try {
    ...
    const folders = db.prepare('SELECT DISTINCT folder FROM saved_posts ORDER BY folder').all() as any[];

    return NextResponse.json({ saved, folders: folders.map(f => f.folder) });
  } catch (e: any) {
    return NextResponse.json({ saved: [], folders: [], error: e.message }, { status: 500 });
  }
}
